...
single flowing app using QStackedWidget.
"""
...
class MainWindow(QMainWindow):
    def __init__(self, model_path: str, speed_limit_kmh: float = 60.0):
        super().__init__()
        self.setWindowTitle("Speed Detection Dashboard")
    ...
    def _on_calibration_finished(self, lines: List[LineData]):
        ...
        # Calibration widget's job is done — remove it from the stack
        self.stacked_widget.removeWidget(self.calibration_view)
        self.calibration_view.deleteLater()
        self.calibration_view = None

    def restart_flow(self):
        """Resets back to source selection. LiveDetectionStage already
        stopped its own worker before emitting restart_requested."""
        self._current_frame = None
        self._current_source = None
        self.stacked_widget.setCurrentWidget(self.source_selector)

    def closeEvent(self, event: QCloseEvent):
        """Guarantees the background thread is stopped before the app
        exits — otherwise Qt can crash or hang on shutdown."""
